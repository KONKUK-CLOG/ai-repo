...
- **LLM SSE**: `POST /internal/v1/llm/execute/stream` — 이름이 분리된 이벤트(`planning`, `answer`, `blog`, …)로 1·2차 LLM 스트림 전송, 마지막에 `complete`로 전체 결과 JSON (동기 `POST .../execute` 유지)
...
- **SSE**: `POST /internal/v1/llm/execute/stream`, `Content-Type: text/event-stream`. 각 프레임은 `event: <이름>` + `data: <JSON 한 객체>` + 빈 줄. **실시간 청크 전달**은 API Gateway REST에서 버퍼링될 수 있으므로 Lambda Function URL 등 **스트리밍 응답을 지원하는 진입점**에서 검증하세요.

#### LLM SSE 이벤트 계약 (Java 등 상위 서버가 파싱)

| `event` | `data` JSON 키 | 설명 |
| --- | --- | --- |
| `started` | `message` | 요청 처리 시작 |
| `planning` | `delta` | 1차 LLM(툴 계획) 텍스트 델타 |
| `planning_done` | `tools`, `message` | `tools`: 선택된 툴 이름 배열 |
| `tool_running` | `tool`, `message` | 툴 실행 시작 |
| `tool_done` | `tool`, `success`, `message` | 툴 실행 종료 |
| `answer` | `delta` | 2차 LLM — 채팅용 답변 스트림 |
| `blog` | `delta` | 2차 LLM — 블로그 마크다운 스트림 |
| `complete` | *(전체)* | `LLMExecuteResult`와 동일: `ok`, `thought`, `tool_calls`, `final_response`, `final_artifact`(`answer`, `blog_markdown`), `model_used` |
| `error` | `message`, 선택 `code` | 파이프라인 실패 |

`POST /internal/v1/llm/execute`(비스트림)는 OpenAI를 **비스트림**으로 호출하며 위 SSE는 사용하지 않습니다.
